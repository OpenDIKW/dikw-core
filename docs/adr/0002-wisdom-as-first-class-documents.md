...
1. **Wrong author.** The user already writes wisdom in Obsidian — first-principles notes, lessons learned, durable opinions — and wants them to participate in retrieve / lint / link graph alongside wiki pages. The candidate / review queue was overhead for the system designer, not value for the writer. Approved candidates were rare because the LLM proposed too aggressively and the user rejected most.
...
3. **Off-disk format.** `wisdom/_candidates/` and the three aggregate files (`principles.md` / `lessons.md` / `patterns.md`) violated the "Obsidian vault is the product" invariant — they were synthetic engine artifacts that an Obsidian user would not naturally write or organize, and they couldn't be hand-edited without the engine clobbering the changes on the next `distill` pass.
...
- `dikw ingest` scans `<root>/wisdom/**/*.md` after the `sources:` scan, hash-idempotent, and runs each file through the same `persist_page` pipeline as wiki pages: `documents` row at `Layer.WISDOM`, chunks, per-version embeddings, outgoing `[[wikilinks]]`, and `provenance` edges from the page's `sources:` frontmatter. Wisdom chunks land in the same `vec_chunks_v<id>` and FTS tables as wiki / source chunks; there is no separate wisdom vec table.
- `dikw client retrieve` returns wisdom chunks tagged `Hit.layer == "wisdom"` alongside wiki / source hits. Callers group, weight, or cite by layer in their own assembly step. `read_page` / `list_links` / `read_provenance` HTTP APIs accept wisdom paths and resolve cross-layer edges (wisdom→wiki, wiki→wisdom, wisdom→source) symmetrically.
- `broken_wikilink` / `orphan_page` / `missing_provenance` / `duplicate_title` lint scans the unified WIKI + WISDOM page set. The orphan inbound counter credits cross-layer wikilinks so a wiki page cited only from wisdom is not falsely flagged.
- One wisdom-specific column: `documents.status` (CHECK-constrained to `draft | published | favorite | archived`, NULL otherwise). Wiki / source rows are clamped to `status = NULL` at both the application layer (`api._to_document`, `persist_page`) and the storage adapter (`upsert_document` in both sqlite and postgres). Lint kind `invalid_wisdom_status` warns when frontmatter declares an out-of-enum value; ingest stays non-blocking.
...
- **+** One persist pipeline (`persist_page`), one retrieve pipeline (`HybridSearcher.search`), one lint pass (`run_lint`), one set of read APIs — wisdom and wiki share semantics end to end.
...
- **+** Wisdom is `[[wikilinkable]]` to and from wiki pages. Cross-layer same-title collisions stay broken so `lint` surfaces the ambiguity (Karpathy's wrong-merge rule).
- **+** On-disk format honors the Obsidian-vault promise. Authors organize by author directory rather than by engine-imposed kind taxonomy.
- **+** Schema slimmed: 4 dedicated wisdom tables removed (`wisdom_items` / `wisdom_evidence` / `wisdom_embed_meta` / `vec_wisdom_v<n>`), 8 Storage Protocol methods removed, 5 CLI subcommands removed (`distill`, `review {list,approve,reject}`), 3 HTTP routes removed (`GET /v1/wisdom` + 2 approve/reject), 6 schema DTOs removed (`WisdomKind`, `WisdomStatus`-old, `WisdomItem`, `WisdomEvidence`, `WisdomEmbeddingRow`, `WisdomVecHit`). `documents.status` adds one column.
...
3. **Multi-tenant wiki (one wiki per author).** Too aggressive for alpha — would require auth, namespacing, cross-vault search rules, and would push the "Obsidian vault is the product" invariant past comfort. The directory-name-as-author shape gives 80% of the value with 5% of the complexity. Revisit post-1.0.
4. **Frontmatter `author:` field instead of directory.** Less Obsidian-native (Obsidian renders directories in the file tree but frontmatter only in a dedicated panel) and requires a per-file convention that's easy to forget. Directory encoding is mechanical and visible. Rejected.
